...
interface Props extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  ...
  /**
   * Whether to hide the clear and close buttons.
   * Optional, defaults to false.
   */
  hideActions?: boolean;
...
export const MultiAsyncSelect = React.forwardRef<MultiAsyncSelectRef, Props>(
  (
    ...
    const tiggerRef = useRef<HTMLDivElement>(null);
    const [tiggerWidth, setTiggerWidth] = React.useState(0);
    ...
    const onResize = () => {
      if (tiggerRef.current) {
        setTiggerWidth(tiggerRef.current.offsetWidth);
      }
    };
    React.useEffect(() => {
      onResize();
    ...
    return (
      ...
      >
        ...
        >
          <Command shouldFilter={!async} style={{ width: tiggerWidth + "px" }}>
            <CommandInput
            ...
            <CommandList>
              ...
              {async && loading && options.length === 0 && (
                <div className="flex justify-center py-6 items-center h-full">
                  <Spinner color="#ffa500" className="relative top-[-1px]" />
                </div>
              )}
              {async ? (
                !loading &&
